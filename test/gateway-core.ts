...
/**
 * Plugin talks DIRECTLY to the camera (no gateway). Credentials are stored in
 * Stream Deck global settings; per-action settings may override (e.g. a profile
 * that baked a specific camera). LAN use; HTTP digest auth handled here since
 * Node's fetch can't do digest.
 */
...
export type Conn = GlobalSettings;
...
export function connFrom(s: GlobalSettings): Conn {
...
export function parseSel(sel: string | undefined): Selection | null {
    if (!sel) return null;
    try { return JSON.parse(sel) as Selection; } catch { return null; }
}
